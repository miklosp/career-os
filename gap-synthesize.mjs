...
// Uses Sonnet via Bifrost. One call. Output: reports/_gap-analysis.md
//
// Usage:
//   node gap-analysis.mjs 3.5 > /tmp/gap-input.json
//   node gap-synthesize.mjs /tmp/gap-input.json

import { readFileSync, writeFileSync } from 'node:fs';

const INPUT = process.argv[2] ?? '/tmp/gap-input.json';
const OUTPUT = 'reports/_gap-analysis.md';
const BIFROST_URL = process.env.BIFROST_URL ?? 'http://localhost:4444';
const MODEL = process.env.BIFROST_MODEL ?? 'anthropic/claude-sonnet-4-6';
...
INPUT: ${data.length} job evaluations (score >= 3.5/5) for the same candidate (Miklós, senior product/design leader). Each entry shows the role and the evaluator's Block A "CV Match" bullets — these already mix matches AND gaps. Your job is to extract and rank the GAPS only.
...
4. Distinguish two gap types:
   - **TRUE GAPS** — things Miklós actually doesn't have (e.g. fintech domain, FDA/medtech regulatory)
   - **SURFACING GAPS** — things he probably has but the CV doesn't make legible (e.g. customer interview counts, mobile experience hidden inside other roles)
...
console.error(`Proxy: ${BIFROST_URL}`);
