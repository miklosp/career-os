...
var (
	...
	reSummary    = regexp.MustCompile(`(?mi)^\*\*Summary:\*\*\s*(.+)`)
...
func ParseApplications(careerOpsPath string) []model.CareerApplication {
	...
	for _, line := range lines {
		...
		rowCounter++
		// Field 0 is the tracker NUM (e.g., "431"). Fall back to the row
		// counter when the row was malformed and didn't carry an integer.
		trackerNum := rowCounter
	...
	// Hydrate Fetched rows from pending tracker-additions TSVs so the
	// dashboard can show score + report link before the user runs merge.
	hydrateFetchedFromTSVs(careerOpsPath, apps)

	return apps
}

// hydrateFetchedFromTSVs scans data/tracker-additions/*.tsv and copies
// score / report / pdf / notes onto matching Fetched rows. Status stays
// "Fetched" — the data is on disk but not yet merged into applications.md,
...
func hydrateFetchedFromTSVs(careerOpsPath string, apps []model.CareerApplication) {
	...
	pendingByNum := make(map[int]pending)
	...
	for _, e := range entries {
		...
		if sm := reScoreValue.FindStringSubmatch(fields[5]); sm != nil {
			p.score, _ = strconv.ParseFloat(sm[1], 64)
		}
		p.hasPDF = strings.Contains(fields[6], "✅")
		if rm := reReportLink.FindStringSubmatch(fields[7]); rm != nil {
			p.reportNumber = rm[1]
			p.reportPath = rm[2]
		}
	...
	if len(pendingByNum) == 0 {
		return
	}

	for i := range apps {
		if NormalizeStatus(apps[i].Status) != "fetched" {
			continue
		}
		p, ok := pendingByNum[apps[i].Number]
		if !ok {
			continue
		}
		if apps[i].Score == 0 && p.score > 0 {
			apps[i].Score = p.score
			apps[i].ScoreRaw = p.scoreRaw
		}
		if apps[i].ReportPath == "" {
			apps[i].ReportPath = p.reportPath
			apps[i].ReportNumber = p.reportNumber
		}
		if !apps[i].HasPDF && p.hasPDF {
			apps[i].HasPDF = true
		}
		if apps[i].Notes == "" && p.notes != "" {
			apps[i].Notes = p.notes
		}
	}
}
...
func enrichFromScanHistory(careerOpsPath string, apps []model.CareerApplication) {
	...
	if _, err := os.Stat(dbPath); err == nil {
		...
		if err == nil {
			defer db.Close()
			rows, err := db.Query(`SELECT url, IFNULL(company,''), IFNULL(title,'') FROM offers`)
			if err == nil {
				defer rows.Close()
				for rows.Next() {
					var url, company, title string
					if rows.Scan(&url, &company, &title) != nil {
						continue
					}
...
func NormalizeStatus(raw string) string {
	...
	case strings.Contains(s, "skipped-location") || strings.Contains(s, "location-skip") || strings.Contains(s, "geo-skip"):
		return "skipped-location"
	case strings.Contains(s, "fetched") || strings.Contains(s, "pending-eval") || s == "queued":
		return "fetched"
...
func LoadReportSummary(careerOpsPath, reportPath string) (summary, location string) {
	...
	if m := reSummary.FindStringSubmatch(text); m != nil {
		summary = cleanTableCell(m[1])
	}

	if m := reLocation.FindStringSubmatch(text); m != nil {
		location = cleanTableCell(m[1])
	}
...
// CleanupDiscardedFiles removes the JD file and any CV html for the given app.
// Returns the list of deleted file paths (empty if nothing was found).
func CleanupDiscardedFiles(careerOpsPath string, app model.CareerApplication) []string {
	if app.ReportNumber == "" {
		return nil
	}
	...
	if entries, err := os.ReadDir(jdsDir); err == nil {
		for _, e := range entries {
			name := e.Name()
			if strings.HasPrefix(name, prefix) && (strings.HasSuffix(name, ".md") || strings.HasSuffix(name, ".txt")) {
				p := filepath.Join(jdsDir, name)
				if os.Remove(p) == nil {
