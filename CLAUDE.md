...
- **`config/`** — user-edited inputs. `cv.json`, `profile.md`, `portals.yml`, `story-bank.md`, `aliases.yml`, `notes.yml`, `cover-letters/` (accepted, candidate-edited letters; `modes/cover-letter.md` matches the newest). Personalization goes HERE.
...
1. **Prep** — `node lib/prep-jds.mjs <url...>` (deterministic, zero-token): per URL, `lib/fetch-jd.mjs` (dedup, ban list, NUM via `lib/next-num.mjs`, save `data/jds/{NUM}-*.md`, insert `Fetched` row) then `lib/location-gate.mjs`. Emits the eval queue as one JSON line. `unknown-host`/`error`/orphan URLs are deferred to **solo agents** — the only path that loads `modes/_fetch.md`, which teaches `lib/ats-registry.json` so the next hit is zero-token.
2. **Location gate** — deterministic rules run inside prep (`Skipped-Location` rows written by the script, quoted JD evidence in Notes). JDs flagged `needs-llm` are gated by the eval agent via `modes/_location-gate.md` vs `config/profile.md` `location_policy` before scoring; on SKIP, no report.
3. **Score** — `modes/_eval.md` in the batch agent: one `node lib/eval-context.mjs <num...>` call supplies the annotated CV, profile, story-bank digest, confirmed notes, report example, and every JD in a single turn; each JD is a **sealed, independent evaluation** (no cross-JD comparison or citation reuse). JD-text-only triage, **zero WebSearch**. A/B/C/D scored; weighted global score in the `**Score:**` header. Writes `data/reports/{NUM}-*.md` + a TSV in `data/tracker-additions/` per JD.
...
- Per-generation audit: `output/customized-cvs/{NUM}-{slug}-trace.json` (bullet→src, `<gaps>`, `<bridges>` with `source_type`, validator chain). The fact-checker consumes this (citation map + gaps); the ATS simulator scores the CV text alone against the report's Criteria ledger (per-criterion meets/uncertain/does-not-meet; met/total and overclaim/uncertain/lost deviations computed in code). Both merge into one review JSON. Report-only — neither injects text.
