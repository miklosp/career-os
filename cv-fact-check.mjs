...
if (!cvArg) {
  console.error("Usage: node cv-fact-check.mjs [--review-only] <cv-path>");
  console.error(
    "Example: node cv-fact-check.mjs output/210-dash0-principal-product-manager-cv.md",
  );
  process.exit(1);
}
...
const jdPath = resolve(__dirname, `jds/${num}-${slug}.md`);
...
const reviewJsonPath = resolve(
  __dirname,
  `output/${num}-${slug}-cv-review.json`,
);
...
if (reviewJsonExists && !reviewOnly) {
  ...
  const [sourceCv, jdText, reviewPrompt] = await Promise.all([
    readFile(resolve(__dirname, "config/cv.md"), "utf-8"),
    readFile(jdPath, "utf-8"),
    readFile(resolve(__dirname, "config/cv-review-prompt.md"), "utf-8"),
  ]);
  ...
  console.log(`${C.dim}   JD:     jds/${num}-${slug}.md${C.reset}`);
  console.log(
...
for (let i = 0; i < findings.length; i++) {
  ...
  console.log(`${C.bold}Generated:${C.reset}  ${f.generated_text}`);
  console.log(`${C.bold}Source CV:${C.reset}  ${f.source_cv_evidence}`);
  console.log(`${C.bold}Issue:${C.reset}      ${f.issue}`);
  console.log("");
  const fixDisplay =
    f.proposed_fix === ""
      ? `${C.dim}(remove the offending text)${C.reset}`
      : f.proposed_fix;
  console.log(`${C.bold}Proposed:${C.reset}   ${fixDisplay}`);
  console.log("");
  ...
  if (answer === "a" || answer === "e") {
    ...
    cvContent = cvContent.replace(f.generated_text, replacement);
...
if (approved + edited > 0) {
  ...
  if (yn === "" || yn === "y" || yn === "yes") {
    const pdfPath = resolve(__dirname, `output/${num}-${slug}-cv.pdf`);
    const cssPath = resolve(__dirname, "templates/cv-template.css");
    try {
      execFileSync(
        "uv",
        [
