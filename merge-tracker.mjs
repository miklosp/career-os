...
const CANONICAL_STATES = ['Fetched', 'Skipped-Location', 'Evaluated', 'Applied', 'Responded', 'Interview', 'Offer', 'Rejected', 'Discarded', 'SKIP'];
...
for (const file of tsvFiles) {
  ...
  if (duplicate) {
    ...
    const promotingFetched = /^fetched$/i.test(duplicate.status || '');

    if (promotingFetched || newScore > oldScore) {
      const nextStatus = promotingFetched ? addition.status : duplicate.status;
      const note = promotingFetched
        ? addition.notes
        : `Re-eval ${addition.date} (${oldScore}→${newScore}). ${addition.notes}`;
      console.log(promotingFetched
        ? `⬆️  Promote: #${duplicate.num} ${addition.company} — Fetched → ${addition.status} (${newScore})`
        : `🔄 Update: #${duplicate.num} ${addition.company} — ${addition.role} (${oldScore}→${newScore})`);
      const lineIdx = appLines.indexOf(duplicate.raw);
      if (lineIdx >= 0) {
        const updatedLine = `| ${duplicate.num} | ${addition.date} | ${addition.company} | ${addition.role} | ${addition.score} | ${nextStatus} | ${duplicate.pdf} | ${addition.report} | ${note} |`;
...
if (VERIFY && !DRY_RUN) {
  ...
  try {
    execFileSync('node', [join(CAREER_OPS, 'lib/verify-pipeline.mjs')], { stdio: 'inherit' });
  } catch (e) {
    process.exit(1);
  }
}
