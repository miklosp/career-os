...
- **Filter Sentry noise by line prefix, not by fragment.** The broad `-iE '...|NSU|NSF|couldn|...'` filter eats eval read-backs whose JSON contains those fragments inside ordinary words: `consumer` matches `NSU` case-insensitively, so on Enode #3484 a whole-form read-back came back with the textarea value silently missing and a bare `exit 1`. The Sentry lines all start with `[Sentry]`; use `grep -v -E '^\[Sentry\]'` instead.
