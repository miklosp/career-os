 ...
 *   node lib/cv-project.mjs --archetype product --out output/cv-product.md
 *
 * Flags:
...
const json = JSON.parse(await readFile(resolve(root, "config/cv.json"), "utf-8"));
...
if (AS_JSON) {
  ...
  const profileRaw = await readFile(resolve(root, "config/profile.md"), "utf-8");
