 ...
 *
 * Inputs (all relative to USER_DIR, see lib/paths.mjs):
...
import { CONFIG_DIR, OUTPUT_DIR, USER_DIR } from "./paths.mjs";
...
const rd = async (p) => {
  try {
    return await readFile(resolve(USER_DIR, p), "utf-8");
  } catch {
    return null;
  }
};
...
if (!cvJson) {
  const report = {
    onboarding: true,
    message:
      "user/config/cv.json is missing. Copy user-template/config/cv.md to user/config/cv.md, " +
  ...
  if (AS_JSON) process.stdout.write(JSON.stringify(report, null, 2) + "\n");
    ...
    console.log("⚠️  No user/config/cv.json — onboarding required.");
...
// 1. Files (5)
{
  ...
  if (sbRaw) pts += 1;
  else findings.push("story-bank.md missing — copy from user-template/config/story-bank.md.");
  if (profileRaw) pts += 1;
  else findings.push("profile.md missing — copy from user-template/config/profile.md.");
  ...
  if (!sbRaw) actions.push({ gain: 1, priority: 85, message: "Initialize story bank", command: "cp user-template/config/story-bank.md user/config/story-bank.md" });
}
...
try {
  ...
  for (const f of await readdir(outDir)) {
    if (/^_keyword-analysis.*\.md$/.test(f)) {
      ...
      try {
        const cvSt = await stat(resolve(CONFIG_DIR, "cv.json"));
