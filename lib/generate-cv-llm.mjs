...
// config/profile.md is markdown with a YAML frontmatter block; parse the
// frontmatter only (fall back to whole-file for a bare-YAML legacy file).
const fmMatch = profileRaw.match(/^---\s*\n([\s\S]*?)\n---/);
const profile = yamlLoad(fmMatch ? fmMatch[1] : profileRaw);
const c = profile.candidate;

// ── Build prompt ─────────────────────────────────────────────────────────────
// lib/prompts/ats-prompt.md uses {cv_content}, {report_content}, {story_bank_content}, {job_content}.
