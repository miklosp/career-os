...
const jdFiles = readdirSync(join(root, "data/jds"));
const jds = nums.map((num) => {
  const file = jdFiles.find((f) => f.startsWith(`${num}-`));
  if (!file) {
    console.error(`eval-context: no JD on disk for NUM ${num} (data/jds/${num}-*.md)`);
    process.exit(1);
  }
  return { num, file, text: readFileSync(join(root, "data/jds", file), "utf8") };
});

function storyBankDigest() {
  const raw = readFileSync(join(root, "config/story-bank.md"), "utf8");
...
out += section("PROFILE (config/profile.md)", readFileSync(join(root, "config/profile.md"), "utf8"));
out += section(
  "STORY BANK DIGEST (S0xx ids citable; full STAR text intentionally omitted)",
  storyBankDigest(),
);
out += section("CONFIRMED NOTES (config/notes.yml)", readFileSync(join(root, "config/notes.yml"), "utf8"));
out += section("REPORT EXAMPLE (templates/report.example.md)", readFileSync(join(root, "templates/report.example.md"), "utf8"));
for (const jd of jds) out += section(`JD ${jd.num} (data/jds/${jd.file})`, jd.text);
