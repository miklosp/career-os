...
// lib/fetch-jd.mjs for the Apify-shaped detail it still handles elsewhere —
...
function buildJobSpyJd({ canonicalUrl, fetchedDate, job, resolution }) {
  ...
  const method = resolution?.kind === 'offsite'
    ? 'jobspy-linkedin+voyager'
    : 'jobspy-linkedin';
  const lines = [];
  ...
  lines.push(`**Remote scope:** ${job.is_remote ? 'full-remote-region:unspecified' : 'unspecified'}`);
  lines.push(`**Timezone:** unspecified`);
  lines.push(`**Visa/authorization:** unspecified`);
  lines.push(`**Relocation offered:** unspecified`);
  lines.push('');
  ...
  for (const job of survivors) {
    ...
    if (cookiesPresent && !voyagerDead) {
      ...
      if (resolution.kind === 'auth-expired') {
        ...
        resolution = null;
      } else if (resolution.kind === 'offsite') {
        stats.resolved++;
      } else if (resolution.kind === 'easy-apply') {
        stats.easyApply++;
      } else {
        stats.deferred++; // unknown/error → fall back to LinkedIn URL
      }
    ...
    // Canonical URL: resolved offsite ATS URL verbatim (query params matter
    // for folderId-style ATSes — do NOT run the lossy canonicalizeUrl on it);
    // otherwise the LinkedIn URL (Easy-Apply, no cookies, or unresolved).
