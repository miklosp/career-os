...
export function canonicalizeUrl(raw) {
  ...
  // Greenhouse embed-widget hosts (careers.<co>.com/?gh_jid=) carry job identity
  // in the query, not the path — keep gh_jid so distinct postings at a shared base
  // path don't collapse to one canonical URL. All other query params are dropped.
  ...
  const query = ghJid ? `?gh_jid=${ghJid}` : '';
