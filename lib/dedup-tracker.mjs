 ...
 * preserves that status. Outcome columns (applied date, channel, furthest
 * stage, rejection reason) are merged via mergeOutcome so a removed row never
 * takes the stage reached with it.
 *
 * Run: node career-ops/dedup-tracker.mjs [--dry-run]
 *
 * The dedup runs in main(), guarded by an entrypoint check at the bottom, so the
 * pure normalization helpers (normalizeCompany, normalizeRole) can be imported
 * without triggering a dedup pass — lib/detect-reposts.mjs reuses them.
 */

import { readFileSync, writeFileSync, copyFileSync, existsSync, mkdirSync } from 'fs';
import { pathToFileURL } from 'url';
import { APPLICATIONS_FILE, DATA_DIR } from './paths.mjs';
...
// Outcome columns after Notes: cells 10–13 of a split row.
const OUTCOME_START = 10;
const OUTCOME_FIELDS = ['appliedDate', 'channel', 'furthestStage', 'rejectionReason'];
// Order matches templates/states.yml outcome.furthest_stage (furthest last).
const STAGE_RANK = ['none', 'screen', 'assessment', 'hm', 'panel', 'final', 'offer'];

// Keeper's non-empty value wins per field; otherwise the first non-empty one
// from the removed rows. furthestStage takes the furthest stage of the cluster.
function mergeOutcome(keeper, others) {
  const merged = { ...keeper };
  for (const f of OUTCOME_FIELDS) {
    if (!merged[f]) merged[f] = others.map(o => o[f]).find(Boolean) || '';
  }
  for (const o of others) {
    if (STAGE_RANK.indexOf(o.furthestStage) > STAGE_RANK.indexOf(merged.furthestStage)) {
      merged.furthestStage = o.furthestStage;
    }
  }
  return merged;
}
...
function parseAppLine(line) {
  ...
  return {
    ...
    report: parts[8],
    notes: parts[9] || '',
    outcome: Object.fromEntries(OUTCOME_FIELDS.map((f, i) => [f, parts[OUTCOME_START + i] ?? ''])),
    raw: line,
  };
}
...
function main() {
  ...
  for (const [company, companyEntries] of groups) {
    ...
    for (let i = 0; i < companyEntries.length; i++) {
      ...
      const outcome = mergeOutcome(keeper.outcome, cluster.slice(1).map(e => e.outcome));
      const outcomeChanged = OUTCOME_FIELDS.some(f => outcome[f] !== keeper.outcome[f]);

      // Update keeper's status if a removed entry had a more advanced one,
      // and its outcome columns if a removed entry carried outcome data.
      if (bestStatus !== keeper.status || outcomeChanged) {
        const lineIdx = entryLineMap.get(keeper.num);
        if (lineIdx !== undefined) {
          const parts = lines[lineIdx].split('|').map(s => s.trim());
          parts[6] = bestStatus;
          const cells = parts.slice(1, OUTCOME_START).concat(OUTCOME_FIELDS.map(f => outcome[f]));
          ...
          if (outcomeChanged) console.log(`  📝 #${keeper.num}: outcome merged from removed duplicates`);
        }
      }
...
export { normalizeCompany, normalizeRole, mergeOutcome };
