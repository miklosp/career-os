...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const numArg = (() => {
    ...
    return i !== -1 ? args[i + 1] : (basename(cvPath).match(/^(\d{3})-/) || [])[1];
  })();

  const { load } = await import("js-yaml");
  const readMaybe = async (p, d = "") => (existsSync(p) ? readFile(p, "utf-8") : d);
  const cvJson = JSON.parse(await readFile(resolve(ROOT, "config/cv.json"), "utf-8"));
  const storyBankMd = await readMaybe(resolve(ROOT, "config/story-bank.md"));
  const notesYml = load(await readMaybe(resolve(ROOT, "config/notes.yml"), "notes: []")) || { notes: [] };
  const aliasesYml = load(await readMaybe(resolve(ROOT, "config/aliases.yml"), "aliases: []")) || { aliases: [] };

  let reportMd = "";
  if (numArg) {
    const rdir = resolve(ROOT, "data/reports");
    if (existsSync(rdir)) {
