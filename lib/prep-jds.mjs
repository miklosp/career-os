...
//   exists, appStatus null   → done when a report exists (row removed by
//                              dedup-tracker as a duplicate; silent);
//                              otherwise deferred (orphaned JD — solo agent
//                              re-registers)
  ...
  switch (fetched.status) {
    ...
    case "exists":
      ...
      if (fetched.appStatus == null) {
        if (hasReport(fetched.num)) {
          out.done += 1;
          return log(i, `#${fetched.num} has a report but no row (deduped) → done`);
        }
