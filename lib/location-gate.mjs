...
const JD_DIR = path.join(DATA_DIR, 'jds');
const PROFILE = path.join(CONFIG_DIR, 'profile.md');
...
function updateAppsRow(num, status, notes) {
  ...
  const m = text.match(new RegExp(`^\\| ${num} \\|.*$`, 'm'));
  if (!m) return { changed: false, reason: 'no row' };
  // Cells: 0='' 1=# … 6=Status … 9=Notes, then the outcome columns — rewrite
  // only Status and Notes so every other column survives. Evidence quotes JD
  // text, which may carry pipes that would split the Notes cell.
  const cells = m[0].split('|');
  if (cells.length < 11) return { changed: false, reason: 'malformed row' };
  cells[6] = ` ${status} `;
  cells[9] = ` ${notes.replace(/\\?\|/g, '/')} `;
  text = text.slice(0, m.index) + cells.join('|') + text.slice(m.index + m[0].length);
