...
// Diff the report's Extracted Keywords against the rendered CV text (and the
// PDF text layer if one exists). We only REPORT absence.
async function keywordSurvival() {
  ...
  const secM = reportTxt.match(/extracted keywords[^\n]*\n([\s\S]*?)(?:\n##|\n#|\n---|\s*$)/i);
  if (!secM) return null;
  const kws = [
    ...new Set(
      secM[1]
  ...
  const missing = kws.filter((k) => !present(k));
  return { total: kws.length, missing };
}
