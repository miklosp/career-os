 ...
 *   REVIEW_MODEL             — ATS-simulation model ID (.env; optional →
 *                              GENERATION_MODEL). Same family as Ashby's
 *                              evaluator by design.
 *   FACTCHECK_MODEL          — citation fact-check model ID (.env; optional →
 *                              REVIEW_MODEL). Keep this a different model
 *                              family from BOTH the generator and REVIEW_MODEL
 *                              so the two checks fail independently.
 *
...
// generate-cv-llm.mjs derives the CV slug from the JD *body heading*, which can
...
async function findJd() {
  const jdsDir = resolve(__dirname, "data/jds");
  if (trace.jd) {
...
if (!jdPath) {
  console.error(`❌ No JD found for NUM ${num} in data/jds/.`);
