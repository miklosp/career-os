 ...
 *     Triggered by --review-only. Used by the tailor mode (dashboard `t`)
 *     so the review runs right after CV generation.
 *
 ...
 *   REVIEW_MODEL             — ATS-simulation model ID (.env; required).
 *                              Same family as Ashby's evaluator by design.
 *   FACTCHECK_MODEL          — citation fact-check model ID (.env; optional →
 *                              REVIEW_MODEL). Keep this a different model
 *                              family from BOTH the drafting session's model
 *                              and REVIEW_MODEL so the checks fail independently.
 *
 * The two reviewer calls run in parallel and merge into one review JSON
 * (findings/summary from FACTCHECK_MODEL, simulation from REVIEW_MODEL) —
 * the on-disk contract consumed by the dashboard is unchanged.
 */
...
import { CONFIG_DIR, DATA_DIR, OUTPUT_DIR } from "./paths.mjs";
import { execFileSync } from "child_process";
import readline from "readline";

// `__dirname` here is the project root (one level above lib/); user data resolves via ./paths.mjs.
...
if (!REVIEW_MODEL) {
  console.error("❌ REVIEW_MODEL is not set — define it in .env (no hardcoded fallback).");
  process.exit(1);
}
const FACTCHECK_MODEL = process.env.FACTCHECK_MODEL ?? REVIEW_MODEL;

// mergeGeneratorBridges folds the draft's <bridges> output (saved in
// {NUM}-{slug}-trace.json by lib/cv-draft.mjs) into the reviewer's findings list. Bridges are
...
if (!cvArg) {
  ...
  console.error(
    "Example: node lib/cv-fact-check.mjs user/output/customized-cvs/210-dash0-principal-product-manager-cv.md",
  );
  process.exit(1);
}
...
// ── Locate the evaluation report (same logic as lib/cv-draft.mjs) ───────────
...
// ── Audit trace (from lib/cv-draft.mjs finalize) ────────────────────────────
...
const tracePath = resolve(OUTPUT_DIR, `customized-cvs/${num}-${slug}-trace.json`);
const bridgesPath = resolve(OUTPUT_DIR, `customized-cvs/${num}-${slug}-cv-bridges.json`);
  ...
  for (const p of [tracePath, bridgesPath]) {
    ...
    try {
      ...
      return {
        ...
        jd: typeof t.jd === "string" ? t.jd : null,
      };
    } catch {
      /* try next */
    }
  }
  return { bridges: [], gaps: [], bullets: [], validator: null, jd: null };
}
const trace = await loadTrace();

// ── Locate the originating JD ───────────────────────────────────────────
// lib/cv-draft.mjs derives the CV slug from the JD *body heading*, which can
// differ from the JD *filename* slug, so the filename is not a reliable key.
// Resolve in order: the JD the generator recorded in the trace, the
// slug-matched path, then the newest JD sharing this NUM.
async function findJd() {
  const jdsDir = resolve(DATA_DIR, "jds");
  if (trace.jd) {
    const recorded = resolve(jdsDir, trace.jd);
    if (existsSync(recorded)) return recorded;
  }
  const bySlug = resolve(jdsDir, `${num}-${slug}.md`);
  if (existsSync(bySlug)) return bySlug;
  if (!existsSync(jdsDir)) return null;
  const candidates = (await readdir(jdsDir)).filter(
    (n) => n.startsWith(`${num}-`) && n.endsWith(".md"),
  );
  if (candidates.length === 0) return null;
  const stats = await Promise.all(
    candidates.map(async (n) => ({
      name: n,
      mtime: (await stat(resolve(jdsDir, n))).mtimeMs,
    })),
  );
  stats.sort((a, b) => b.mtime - a.mtime);
  return resolve(jdsDir, stats[0].name);
}

const jdPath = await findJd();
if (!jdPath) {
  console.error(`❌ No JD found for NUM ${num} in user/data/jds/.`);
  console.error(`   Cannot fact-check without the originating JD.`);
  process.exit(1);
}

// ── Review JSON path ─────────────────────────────────────────────────────────
const reviewJsonPath = resolve(OUTPUT_DIR, `customized-cvs/${num}-${slug}-cv-review.json`);
...
if (reviewJsonExists && !reviewOnly) {
  ...
  const [cvJsonRaw, jdText, reviewPrompt, simPrompt, reportMd, storyBankMd, notesRaw, aliasesRaw] =
    await Promise.all([
      ...
      readFile(resolve(__dirname, "lib/prompts/ats-sim-prompt.md"), "utf-8"),
      reportPath
        ? readFile(reportPath, "utf-8")
        : Promise.resolve("(no evaluation report found — Block A grounding unavailable)"),
      readMaybe(resolve(CONFIG_DIR, "story-bank.md"), "(no story bank)"),
  ...
  const filledFactPrompt = reviewPrompt
  ...
  const filledSimPrompt = simPrompt
  ...
  console.log(`${C.dim}   JD:     ${relative(__dirname, jdPath)}${C.reset}`);
  console.log(
    `${C.dim}   Models: sim ${REVIEW_MODEL} · fact-check ${FACTCHECK_MODEL} via ${REVIEW_URL}${C.reset}`,
  );
  console.log("");
  ...
  async function callJsonModel(model, prompt, label) {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      const response = await fetch(`${REVIEW_URL}/v1/chat/completions`, {
        ...
        body: JSON.stringify({
          ...
          // JSON body; each call emits a single stage now, but keep headroom
          // for reasoning + output (16384 was the working single-stage budget).
          max_tokens: 16384,
      ...
      if (!response.ok) {
        ...
        if (finishReason === "length") {
          ...
          lastErr = `${label} truncated at max_tokens (finish_reason:length) — raise max_tokens or lower the model's reasoning for ${model}`;
        } else if (!raw) {
          lastErr = `Empty response from ${label} (choices: null)`;
        } else {
          const usage = payload.usage;
          if (usage) {
            console.log(
              `${C.dim}📊 ${label} tokens: ${usage.prompt_tokens ?? "?"} in / ${usage.completion_tokens ?? "?"} out${C.reset}`,
            );
          }
          raw = raw
            .replace(/^```(?:json)?\s*\n/, "")
          ...
          try {
            ...
            lastErr = `Invalid JSON from ${label}: ${e.message}`;
          }
        }
      }
    ...
    throw new Error(lastErr || `Unknown ${label} error`);
  }

  // Two independent checks, two model families, run in parallel:
  // findings/summary from the fact-checker, simulation from the ATS simulator.
  let factReview, simReview;
  try {
    [factReview, simReview] = await Promise.all([
      callJsonModel(FACTCHECK_MODEL, filledFactPrompt, "fact-check"),
      callJsonModel(REVIEW_MODEL, filledSimPrompt, "ATS simulation"),
    ]);
  } catch (e) {
  ...
  review = { ...factReview };
  if (simReview?.simulation) review.simulation = simReview.simulation;
...
if (approved + edited > 0) {
  ...
  if (yn === "" || yn === "y" || yn === "yes") {
    const pdfPath = resolve(OUTPUT_DIR, `customized-cvs/${num}-${slug}-cv.pdf`);
