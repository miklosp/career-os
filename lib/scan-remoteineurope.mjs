...
import { CONFIG_DIR, SCAN_HISTORY_DB } from './paths.mjs';
...
export async function runRemoteInEuropeScan({
  ...
  await pLimit(FETCH_CONCURRENCY, newUrls, async (sourceUrl) => {
    ...
    if (!passesTitleFilter(role, titleFilter)) {
      stats.skippedTitle++;
      if (!dryRun) recordOffer(db, { url: sourceUrl, firstSeen: fetchedDate, portal: 'remoteineurope', title: role, company, status: 'skipped_title' });
      return;
    }
    ...
    if (isBanned({ company, url: `${applyUrl || ''} ${canonical}` })) {
      stats.banned++;
      if (!dryRun) recordOffer(db, { url: sourceUrl, firstSeen: fetchedDate, portal: 'remoteineurope', title: role, company, status: 'banned' });
      return;
    }

    if (!dryRun) {
      recordOffer(db, { url: sourceUrl, firstSeen: fetchedDate, portal: 'remoteineurope', title: role, company, status: 'added' });
    }
