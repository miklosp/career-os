...
//   - data/jds/NNN-*.md            (JD files written at fetch time)
//   - data/jds/NNN.reserved        (active reservations from concurrent agents)
//   - data/reports/NNN-*.md   (evaluation reports)
...
//
// `nextNum()` atomically reserves a NUM by creating `data/jds/{NUM}.reserved`
...
// writing the real `data/jds/{NUM}-{slug}.md`. Orphaned markers are harmless:
...
// `cwd` = the user-data root holding data/ (lib/paths.mjs).
const DEFAULT_CWD = USER_DIR;
...
export function currentMax({ cwd = DEFAULT_CWD } = {}) {
  return Math.max(
    maxFromDir(join(cwd, 'data', 'jds')),
    maxFromDir(join(cwd, 'data', 'reports')),
...
export function nextNum({ cwd = DEFAULT_CWD, maxAttempts = 100 } = {}) {
  const jdsDir = join(cwd, 'data', 'jds');
  ...
  throw new Error(
    ...
    `check user/data/jds/ for stale .reserved files`,
  );
}

export function releaseNum(num, { cwd = DEFAULT_CWD } = {}) {
  const reservation = join(cwd, 'data', 'jds', `${num}.reserved`);
