...
//
// Parses the "Extracted Keywords" (legacy) or "Criteria" section of every
...
import { CONFIG_DIR, DATA_DIR, OUTPUT_DIR, USER_DIR } from './paths.mjs';
...
// "Criteria" ledger (each criterion minus its `[evidenced]`/`[gap]` tag and
// its `— [src: id]` tail). Every returned string is fed through
// atomize()/canon() exactly like a legacy bullet line, so both formats share
// one counting path.
function reportKeywordSources(lines) {
  ...
  const criteria = lines.findIndex((l) => /^#+\s+Criteria/i.test(l));
  if (criteria !== -1) {
    for (let i = criteria + 1; i < lines.length; i++) {
      const l = lines[i];
      if (/^#+\s/.test(l)) break;              // next section
      if (!/^\s*[-*]\s+/.test(l)) continue;    // only bullets (skip intro lines)
      let term = l.replace(/^\s*[-*]\s+/, '').trim();
      term = term.replace(/^\[(?:evidenced|gap)\]\s*/i, ''); // drop status tag
...
function loadCorpus(rel, extra = '') {
  let t = '';
  try { t = readFileSync(join(USER_DIR, rel), 'utf8'); } catch { /* optional */ }
  return canon(t + ' ' + extra);
}
let skillsLine = '';
try {
  const cvj = JSON.parse(readFileSync(join(CONFIG_DIR, 'cv.json'), 'utf8'));
...
if (!TOP && !ONLY_GAPS && MIN_FREQ <= 1) {
  ...
  console.log(`\nArtifact: ${outPath} (top 60 + ${gaps.length} high-freq gaps)`);
}
