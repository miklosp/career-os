...
//
// Parses the "Extracted Keywords" section of every data/reports/*.md, normalizes
// and de-dupes terms, counts how many distinct reports each appears in, then
...
for (const f of files) {
  ...
  const start = lines.findIndex((l) => /^#+\s+Extracted Keywords/i.test(l));
  if (start === -1) continue;
  ...
  for (let i = start + 1; i < lines.length; i++) {
    ...
    if (!/^\s*[-*]\s+/.test(l)) continue;  // only bullets
    for (const atom of atomize(l)) {
