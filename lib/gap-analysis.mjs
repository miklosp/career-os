...
function parseApplications() {
  const raw = readFileSync(join(DATA_DIR, 'applications.md'), 'utf8');
  const rows = [];
...
function findReport(num) {
  ...
  const matches = readdirSync(join(DATA_DIR, 'reports')).filter(f => f.startsWith(padded + '-'));
...
for (const app of apps) {
  ...
  if (!fname) continue;
  const bullets = extractBlockA(join(DATA_DIR, 'reports', fname));
