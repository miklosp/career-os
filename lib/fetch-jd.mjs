...
// ── .env (APIFY_API_TOKEN for the LinkedIn handler) ──────────────────
...
const handlers = {
  ...
  linkedin: {
    ...
    async fetch({ id }) {
      ...
      if (!token) throw new Error('APIFY_API_TOKEN not set — cannot resolve LinkedIn');
      const res = await fetch(
        `https://api.apify.com/v2/acts/apimaestro~linkedin-job-detail/run-sync-get-dataset-items?token=${encodeURIComponent(token)}`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ job_id: [String(id)] }),
        },
      );
      if (!res.ok) throw new Error(`Apify HTTP ${res.status}: ${(await res.text()).slice(0, 160)}`);
      const detail = (await res.json())[0];
      if (!detail) throw new Error('Apify returned no record for this job id');
      const a = detail.apply_details || {};
      const linkedinUrl = `https://www.linkedin.com/jobs/view/${id}`;
      const canonical = canonicalizeUrl(a.application_url)
        || canonicalizeUrl(linkedinUrl);
      // LinkedIn has its own correct markdown writer in scan-linkedin.mjs.
      return {
        _linkedinDetail: detail,
        canonicalUrl: canonical,
        company: detail.company_info?.name?.trim() || 'Unknown Company',
        role: detail.job_info?.title?.trim() || 'Unknown Role',
      };
    },
  },
};
...
async function cmdFetch(url, { dryRun }) {
  ...
  // already-saved URL must not re-hit the upstream — that burns Apify
  // credits for LinkedIn and spuriously `error`s if the source has since
  ...
  // known after the Apify call, so a *different* LinkedIn id that resolves
