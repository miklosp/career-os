...
import { cleanAtsUrl } from './li-voyager.mjs';
...
for (const d of drop) copyFileSync(d.path, join(bakDir, d.file));
console.log(`\nbackup → ${bakDir}`);

// 5a. JD URL rewrites (skip files being dropped)
const dropPaths = new Set(drop.map(d => d.path));
let rewrote = 0;
for (const j of urlRewrites) {
